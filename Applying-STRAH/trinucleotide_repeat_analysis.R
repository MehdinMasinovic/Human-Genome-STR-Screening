...
"
 ...
 Here, I show how I applied STRAH on the full set of distinct trinucleotide repeats. 
 The number of distinct combinations is 60, therefore, the process of running STRAH is automated via a function (applying_STRAH()).
...
applying_STRAH <- function(letter){
  ...
  for(i in 1:nrow(pos_matrix)) {
    ...
    starts <- Sys.time()
    #Apply Strah
    res <- STR_analysis(nr.STRs = 2, max.nr.STRs = 200, nr.mismatch = 0, STR = letter, addToHs = 500, lens.grey = 0:5*1000,  reverse.comp = FALSE, pos_matrix = pos_matrix[i,], species=BSgenome.Hsapiens.UCSC.hg38::Hsapiens)
    ends <- Sys.time()
...
####Generate the pool of trinucleotide repeats####

letters <- c("A", "C", "G", "T")
microsatellites <- do.call(paste0, expand.grid(rep(list(letters),3)))
repeated <- c("AAA", "CCC", "GGG", "TTT")
for(i in repeated) microsatellites <- microsatellites[-(which(microsatellites == i))]
...
microsatellites <- microsatellites[1:20]
#microsatellites <- microsatellites[21:40]
#microsatellites <- microsatellites[41:60]
