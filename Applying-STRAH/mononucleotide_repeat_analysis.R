...
"
 ...
 Here, I show how I applied STRAH on the full set of distinct mononucleotide repeats. 
 The number of distinct combinations is 4, therefore, the process of running STRAH is automated via a function (applying_STRAH()).
...
applying_STRAH <- function(letter){
  ...
  for(i in 1:nrow(pos_matrix)) {
    ...
    starts <- Sys.time()
    #Apply Strah
    res <- STR_analysis(nr.STRs = 6, max.nr.STRs = 200, nr.mismatch = 0, STR = letter, addToHs = 500, lens.grey = 0:5*1000,  reverse.comp = FALSE, pos_matrix = pos_matrix[i,], species=BSgenome.Hsapiens.UCSC.hg38::Hsapiens)
    ends <- Sys.time()
